...
def detect(save_img=False):
    ...
    # Directories
    save_dir = Path(increment_path(Path("../Results") / opt.name, exist_ok=opt.exist_ok))
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            try:
                im0 = cv2.putText(im0, "" %(1/(t2-t1)), (30,30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,0), 2, cv2.LINE_AA)
            except:
                pass
...
if __name__ == '__main__':
    ...
    print('[INFO] Начало трансляции видео')
    with torch.no_grad():
        if opt.update:
